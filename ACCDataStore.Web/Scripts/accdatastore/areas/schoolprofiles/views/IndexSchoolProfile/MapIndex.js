...
function SearchData(sCondition, sKeyname) {
    ...
    $.ajax({
        type: "POST",
        url: sContextPath + "SchoolProfile/EthnicBackground/SearchByName",
            ...
            .highcharts(
                    {
                        ...
                        yAxis: {
                            min: 0,
                            title: {
                                text: '% Pupils in Each EthnicBackground'
                            }
                        },
                        tooltip: {
...
function ShowPopupInfo(data) {
    ...
    sInformation += " <h4 class='text-center'> Ethnicbackground - " + data.dataTitle + "</h4>";
    sInformation += "</div><div class='panel-body'>";
    sInformation += "<table class='table table-bordered table-hover'>";
    sInformation += "<thead><tr><th class='text-center'>Ethnicbackground</th><th class='text-center'>Female</th><th class='text-center'>Male</th><th class='text-center'>Total</th></tr></thead>";
    sInformation += "<tbody>";
    if (data.dataSeries.length != 0) {
        for (var i = 0; i < data.dataSeries.length; i++) {
            sInformation += "<tr><td class='text-left'>" + data.dataSeries[i].EthinicName + "</td><td  align='center'>" + data.dataSeries[i].PercentageFemaleAllSchool.toFixed(2) + "</td><td  align='center'>" + data.dataSeries[i].PercentageMaleAllSchool.toFixed(2) + "</td><td  align='center'>" + data.dataSeries[i].PercentageAllSchool.toFixed(2) + "</td><tr>";
