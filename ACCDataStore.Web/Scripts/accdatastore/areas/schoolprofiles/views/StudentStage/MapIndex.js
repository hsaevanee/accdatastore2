...
function SearchData(sCondition,sKeyname) {
    ...
    $.ajax({
        type: "POST",
        url: sContextPath + "SchoolProfile/StudentStage/SearchByName",
...
function myFunctionColumn(pdata) {
        $.ajax({
            type: 'POST',
            url: sContextPath + 'SchoolProfile/StudentStage/GetChartDataStudentStageforMap',
...
function ShowPopupInfo(data) {
    ...
    sInformation += "<h4 class='text-center'> Student Stage - " + data.dataTitle + "</h4>";
    sInformation += "</div><div class='panel-body'>";
    sInformation += "<table class='table table-bordered table-hover'>";
    sInformation += "<thead><tr><th class='text-center'>Stages</th><th class='text-center'>Female</th><th class='text-center'>Male</th><th class='text-center'>Total</th></tr></thead>";
    sInformation += "<tbody>";
    if (data.dataSeries.length != 0) {
        for (var i = 0; i < data.dataSeries.length; i++) {
            sInformation += "<tr><td class='text-center'>" + data.dataSeries[i].StageCode + "</td><td  class='text-center'>" + data.dataSeries[i].PercentageFemaleAllSchool.toFixed(2) + "</td><td  class='text-center'>" + data.dataSeries[i].PercentageMaleAllSchool.toFixed(2) + "</td><td  class='text-center'>" + data.dataSeries[i].PercentageAllSchool.toFixed(2) + "</td><tr>";
